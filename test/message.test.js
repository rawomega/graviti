...
module.exports = {
	...
	"stringifying a message" : testCase({
		...
		"stringify a message with dest uri and content" : function(test) {
			...
			test.equal(JSON.stringify(this.content), lines[5]);
			test.done();
		},
	...
	"parsing an ack" : testCase({
		"should recognise a valid ack" : function(test) {
			var res = message.parseAck('ACK 123');			
		...
		"should not parse non-ack" : function(test) {
			var res = message.parseAck('BACK 123');
		...
		"should not parse nothing" : function(test) {
			var res = message.parseAck(undefined);
	...
	"parsing a message" : testCase({
		...
		"should throw on parsing if bad method" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /unsupported method/i);
			test.done();
		},
		...
		"should throw on parsing if missing uri" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /missing destination uri/i);
			test.done();
		},
		...
		"should throw on parsing if header with no colon" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /bad header/i);
			test.done();
		},
		...
		"should throw on parsing if headers not done" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /parse headers/i);
			test.done();
		},
		...
		"should throw on parsing if no content" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /expected content/i);
			test.done();
		},
		...
		"should throw on parsing if json content invalid" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /parse json content/i);
			test.done();
		},
		...
		"should throw on parsing if message size exceeded" : function(test) {
			...
			assert.throws(function() { message.parse(str); }, /message size/i);
			test.done();
		}
	}),
	...
	"parsing a partial message" : testCase({
		...
		"should parse one-line partial message" : function(test) {
			...
			var res = message.progressiveParse(str);
		...
		"should parse message with method + uri in 2 parts" : function(test) {
			...
			var res1 = message.progressiveParse(str1);
			var res2 = message.progressiveParse(str2, res1);
		...
		"should throw on parsing if bad method" : function(test) {
			...
			assert.throws(function() { message.progressiveParse(str); }, /unsupported method/i);
			test.done();
		},
		...
		"should throw on parsing if no uri" : function(test) {
			...
			assert.throws(function() { message.progressiveParse(str); }, /missing destination uri/i);
			test.done();
		},
		...
		"should throw on missing header value in 2 parts" : function(test) {
			...
			assert.throws(function() { message.progressiveParse(str2, res1); }, /bad header/i);
			test.done();
		}
	})
};