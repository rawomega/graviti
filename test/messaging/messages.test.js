...
module.exports = {
	"creating a message" : testCase({
		...
		"creating a message with no dest uri" : function(test) {
			assert.throws(function() { new messages.Message(); }, /missing destination uri/i);
			test.done();
		},
		
		"creating a message with only dest uri" : function(test) {
			node.nodeId = myId;
			var msg = new messages.Message('p2p:myuri/myres');
		...
		"creating a message with dest uri and blank dest id" : function(test) {
			node.nodeId = myId;
			var msg = new messages.Message('p2p:myuri/myres', undefined, undefined, 'NIL');
		...
		"creating a message with dest uri and content" : function(test) {
			var msg = new messages.Message('p2p:myuri/myres', {a : 'ay', b : 'bee'});
		...
		"creating a message with dest uri and a mix of additional and overridden headers" : function(test) {
			var msg = new messages.Message('p2p:myuri/myres', undefined, {method : 'POST', myheader : 'moo'});
		...
		"creating a message with dest uri, content and a mix of additional and overridden headers" : function(test) {
			var msg = new messages.Message('p2p:myuri/myres', {a : 'ay', b : 'bee'}, {method : 'POST', myheader : 'baa'});
		...
		"creating a message with a specified dest id" : function(test) {
			var msg = new messages.Message('p2p:myuri/myres', {a : 'ay', b : 'bee'}, {method : 'POST', myheader : 'baa'}, 'AAAAABBBBBAAAAABBBBBAAAAABBBBBAAAAABBBBB');
	...
	"stringifying a message" : testCase({
		...
		"stringify a message with only dest uri" : function(test) {
			var str = new messages.Message('p2p:myuri/myres').stringify();
			var lines = str.split('\n');
		...
		"stringify a message with dest uri and content" : function(test) {
			var str = new messages.Message('p2p:myuri/myres', this.content).stringify();
			var lines = str.split('\n');
		...
		"stringify a message with dest uri, content and custom headers" : function(test) {
			var str = new messages.Message('p2p:myuri/myres', this.content, {custom: 'header'}).stringify();
			var lines = str.split('\n');
		...
		"stringify a message for a specific dest id" : function(test) {
			var str = new messages.Message('p2p:myuri/myres', this.content, {custom: 'header'}, 'AAAAABBBBBAAAAABBBBBAAAAABBBBBAAAAABBBBB').stringify();
			var lines = str.split('\n');
		...
		"stringify a message with non-json content" : function(test) {
			var str = new messages.Message('p2p:myuri/myres', '{not-json}', {content_type: 'text/plain'}).stringify();
			var lines = str.split('\n');
		...
		"fail to stringify a message that is too big" : function(test) {
			...
			assert.throws(function() {
				 new messages.Message('p2p:myuri/myres', '{"a": "b"}', {content_type: 'text/plain'}).stringify();
			}, /size too big/i);
			test.done();
		},
	}),
