...
var self = module.exports = {
	...
	getNextHop : function(id) {
		...
		if (rowForDigit) {
			...
			if (routeForDigit) {
				return {
					next_hop_id   : routeForDigit.id,
					next_hop_addr : routeForDigit.ap.split(':')[0],
					next_hop_port : routeForDigit.ap.split(':')[1]
				};
			}
		}
		...
		if (res) {
			if (res.nearest === node.nodeId) {
				...
				return {
					next_hop_id   : res.nearest,
					next_hop_addr : fallbackRoutingCandidates[res.nearest].split(':')[0],
					next_hop_port : fallbackRoutingCandidates[res.nearest].split(':')[1]
				};
			}
		} else {
			console.log('cc');
