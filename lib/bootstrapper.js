...
var self = module.exports = {
	...
	start : function(overlayCallback, bootstraps) {
		...
		//overlayCallback.on("graviti-message-received", function() {console.log('bbb')});
	...
	_handleReceivedGravitiMessage : function(msg, msginfo) {
		...
		if (/\/bootstraptarget/.test(msg.uri)) {
			...
			var content = {
					leafset : self.overlayCallback.leafset,
					id : node.nodeId
			};
			...
			// send our reply
			self.overlayCallback.sendToAddr('p2p:graviti/bootstrapsource', content, undefined,
	...
	_sendPendingBootstrapRequests : function() {
		...
		for (var endpoint in self.bootstrapEndpoints) {						
			...
			if (endpointData && endpointData.last_attempt_at < (now - self.bootstrapRetryIntervalMsec)) {
				...
				self.overlayCallback.sendToAddr('p2p:graviti/bootstraptarget', content, undefined, endpointParts[0], endpointParts[1]);
