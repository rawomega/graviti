...
var self = module.exports = {
	...
	_sendHeartbeatToLeafsetPeer : function(id, peer) {
		util.log('DEBUG: heartbeatInterval: ' + self.heartbeatIntervalMsec + ', peer: ' + JSON.stringify(peer));
		var lastHeartbeatSent = peer.lastHeartbeatSent;
		if (lastHeartbeatSent > (new Date().getTime() - self.heartbeatIntervalMsec))
			return;
		
		util.log('Heartbeating to leafset peer ' + id + ' (' + peer.ap + ')');
		var ap = peer.ap.split(':');
		var now = new Date().getTime();
		self._sendHeartbeatToAddr(ap[0], ap[1], {
	...
	_sendHeartbeatToAddr : function(addr, port, content) {
	...
	_handleReceivedHeartbeat : function(msg, msginfo) {
		...
		if (msg.content.rsvp_with !== undefined) {
			...
			util.log('Responding to heartbeat from ' + msg.source_id + ' (' + msginfo.sender_addr + ':' + msginfo.sender_port + ')');
			self._sendHeartbeatToAddr(msginfo.sender_addr, msginfo.sender_port, content);
			...
			var roundTripTimeMillis = new Date().getTime() - provisionalPeer.lastProbedAt;
			routingtable.updateWithKnownGood(msg.source_id, msginfo.sender_addr + ':' + msginfo.sender_port, roundTripTimeMillis);
		}
	},
	...
	_probeCandidateRoutingTablePeers : function() {
		routingtable.eachCandidate(function(id, peer) {
			...
			util.log('Probing routing table candidate ' + id + ' (' + peer.ap + ')'); 
			var ap = peer.ap.split(':');
			var now = new Date().getTime();
			self._sendHeartbeatToAddr(ap[0], ap[1], {
	...
	_initiateRoutingTableMaintenance : function() {
		routingtable.eachRow(function(row, peers) {
			...
			routingtable.updateWithProvisional(peer.id, peer.ap);
		});
	}
};