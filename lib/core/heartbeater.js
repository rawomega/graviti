...
var self = module.exports = {
	...
	_handleReceivedHeartbeat : function(msg, msginfo) {
		...
		leafset.updateWithKnownGood(msg.source_id, msginfo.sender_ap);
		...
		if (msg.content.rsvp_with !== undefined) {
			...
			util.log('Responding to heartbeat from ' + msg.source_id + ' (' + msginfo.sender_ap + ')');
			var ap = msginfo.sender_ap.split(':');
			...
			routingtable.updateWithKnownGood(msg.source_id, msginfo.sender_ap, roundTripTimeMillis);
		}
	},
	
	_handleDepartingPeer : function(msg, msginfo) {
		var id = msg.uri.replace(/.*\/peers\//, '').replace(/\//, '');
		util.log('Received departing peer notice for ' + id + ' (' + msginfo.sender_ap + ')');
