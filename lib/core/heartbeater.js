...
var self = module.exports = langutil.extend(new events.EventEmitter(), {
	...
	stop : function() {
		...
		for (var id in leafset) {
			var ap = leafset[id].split(':');
	...
	_sendPendingHeartbeats : function() {
		...
		var leafset = leafsetmgr.internalLeafset();
		for (var targetId in leafset) {
			var lastHeartbeatSent = leafset[targetId].lastHeartbeatSent;
			...
			var ap = leafset[targetId].ap.split(':');
			self.overlayCallback.sendToAddr('p2p:graviti/heartbeat', content, {method : 'POST'}, ap[0], ap[1]);
			leafset[targetId].lastHeartbeatSent = new Date().getTime();
		}
	},
	...
	_purgeTimedOutPeers : function() {
		var leafset = leafsetmgr.internalLeafset();
		for (var id in leafset) {
			if (leafset[id].lastHeartbeatReceived < (new Date().getTime() - self.timedOutPeerIntervalMsec)) {
				util.log('Found timed out peer ' + id + ' (' + leafset[id].ap + ')');
				leafsetmgr.remove(id);
				self.emit('peer-departed', id);
			}
		}
