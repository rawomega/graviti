...
var self = module.exports = {
	...
	findBetterRoutingHop : function(sourceId, destId) {
		var deepestSharedRow = mod_id.getCommonPrefixLength(node.nodeId, sourceId);
		for (var rowIndex = deepestSharedRow; rowIndex >= 0; rowIndex--) {
			...
			console.log('res: ' + JSON.stringify(res) + ' set: ' + JSON.stringify(ids));
			if (res.nearest !== node.nodeId)
				return row[res.nearest[rowIndex]];
		}
		return undefined;
	}
};