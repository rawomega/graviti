...
var self = module.exports = {
	...
	updateWithProvisional : function(a, b, overrideIfKnownGoodExists) {
		...
		for (var id in nodes) {
			...
			winston.verbose('Adding route ' + id + ' (' + nodes[id] + ') to routing candidate set: ' + JSON.stringify(self._candidatePeers));
		...
		for (var digit = 0; digit < mod_id.lengthBits / 4; digit++) {
			...
			if (row && row[currentIdDigit]) {
				if (row[currentIdDigit].id === id) {
					winston.verbose('Updating existing routing table entry for row ' + digit + ', dgt ' + currentIdDigit
					...
					winston.verbose('Existing routing table entry for row ' + digit + ', dgt ' + currentIdDigit + ' has rtt '
			...
			winston.verbose('Routing table entry [row ' + digit + ', dgt ' + currentIdDigit + '] set to ' + JSON.stringify(row[currentIdDigit]));
			break;
		}
	},
