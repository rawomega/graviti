...
winston.verbose('beginning routing - leafset is ' + JSON.stringify(leafset.compressedLeafset()));
	if (node.nodeId === id)
			return {
				id   : node.nodeId
			};
...
winston.verbose('leafset routing hop for dest ' + id + ': ' + JSON.stringify(leafsetHop));
			return leafsetHop;
		}
		...
		if (rowForDigit) {
			...
			if (routeForDigit) {
winston.verbose('routing table hop for dest ' + id + ': ' + JSON.stringify(routeForDigit));
				return {
					id   : routeForDigit.id,
...
winston.verbose('fallback next hop routing to ' + id + ': '  + JSON.stringify(res)	+ ' obtained from fallback routing candidates ' + JSON.stringify(fallbackRoutingCandidates));
