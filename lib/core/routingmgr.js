...
var self = module.exports = {
	...
	getNextHop : function(id) {
		...
		if (leafsetHop !== undefined) {
			//util.log('leafset routing hop for dest ' + id + ': ' + JSON.stringify(leafsetHop));
			return leafsetHop;
		}
		...
		if (rowForDigit) {
			...
			if (routeForDigit) {
				//util.log('routing table hop for dest ' + id + ': ' + JSON.stringify(routeForDigit));
				return {
					id   : routeForDigit.id,
		...
		// util.log('fallback next hop routing to ' + id + ': '  + JSON.stringify(res)	+ ' obtained from fallback routing candidates ' + JSON.stringify(fallbackRoutingCandidates));
