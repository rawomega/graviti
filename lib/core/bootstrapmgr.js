...
// TODO: make sure we cancel any existing ongoing bootstraps when we launch new ones	
	//
	...
	_sendPendingBootstrapRequests : function() {
		...
		for (var endpoint in self.bootstrapEndpoints) {						
			...
			var bootstrapFrom = function(targetNode) {
			...
			if (self.usePns) {
				pns.findNearestNode(endpoint, node.nodeId, function(id, addrPort) {
					util.log('PNS from ' + node.nodeId + ' via seed ' + endpoint + ' gave nearest node '
							+ id + ' (' + addrPort + ')');
					bootstrapFrom(addrPort);
				});
			} else {
				bootstrapFrom(endpoint);
			}
		}
	},
