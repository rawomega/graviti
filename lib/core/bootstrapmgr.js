...
var self = module.exports = {
	...
	//
	// When done bootstrapping, we need to send a copy of our state tables to every peer that
	// appears in them (pastry paper sect 2.4)
	_sendStateTablesToPeers : function() {
		var peers = [];
		leafset.each(function(id, peer) {
			if (peers.indexOf(peer.ap) < 0)
				peers.push(peer.ap);
		});
		routingmgr.each(function(peer) {
			if (peers.indexOf(peer.ap) < 0)
				peers.push(peer.ap);
		});
