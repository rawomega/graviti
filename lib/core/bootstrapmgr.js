...
var self = module.exports = {
	...
	_pnsBootstrapFrom : function(endpoint, endpointData) {
		...
		pns.findNearestNode(endpoint, node.nodeId, function(res) {
			util.log('PNS from ' + node.nodeId + ' via seed ' + endpoint + ' gave nearest node ' + JSON.stringify(res));

			if (endpointData.pns_nearest === undefined || endpointData.pns_nearest.rtt > res.rtt) {
				util.log('Found new nearest node for PNS with rtt ' + res.rtt + ', replacing ' + JSON.stringify(endpointData.pns_nearest));
				endpointData.pns_nearest = {
					id : res.id,
					ap : res.ap,
					rtt : res.rtt
				};
			...
			if (res.public_seed_ap && endpointData.pns_used_seeds.indexOf(res.public_seed_ap) < 0)
				endpointData.pns_used_seeds.push(res.public_seed_ap);
			
			self._addDiscoveredPeers(endpointData, res.discovered_peers);
			...
			if (numDiscoveredPeers < 1 || endpointData.pns_attempt >= self.maxPnsAttempts) {
				self._bootstrapFrom(endpointData.pns_nearest.ap);
			} else {
