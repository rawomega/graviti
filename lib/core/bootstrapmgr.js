...
var self = module.exports = {
	...
	_handleBootstrapRequest : function(msg, msginfo, forwarding) {
		util.log('Bootstrap message for joining node ' + msg.content.joining_node_id + ' from ' + msg.source_id + ' (' + msginfo.sender_addr + ':' + msginfo.sender_port + ')');		
		...
		msg.content.bootstrap_source_addr = bootstrapSourceAddr;
		msg.content.bootstrap_source_port = bootstrapSourcePort;
	...
	//
	// a bootstrap response may be a final one in response to our bootstrap request,
	...
	_handleBootstrapResponse : function(msg, msginfo) {
		util.log('Bootstrap response from ' + msg.source_id + ' (' + msginfo.sender_addr + ':' + msginfo.sender_port + ')');
		...
		routingtable.updateWithKnownGood(msg.source_id, msginfo.sender_addr + ':' + msginfo.sender_port);
		
		// see if this message means we're done bootstrapping
	...
	_sendStateTablesToPeers : function() {
		...
		Object.keys(destinations).forEach(function(addrPort) {
			util.log('Sending state tables to leafset and routing table peers upon bootstrap completion');
			var ap = addrPort.split(':');
			self.overlayCallback.sendToAddr('p2p:graviti/peers', destinations[addrPort],
