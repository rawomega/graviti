...
var self = module.exports = {
	...
	start : function(overlayCallback, bootstraps) {
		if (bootstraps && bootstraps.length > 0) {
			...
			winston.info('=== Going to start a new ring as ' + node.nodeId + ' (empty bootstrap list) ===');
		}
	...
	_startBootstrapping : function(bootstraps) {
		winston.info('Going to join overlay through bootstrap(s) ' + bootstraps);
	...
	_bootstrapFrom : function(targetNode) {
		winston.info('Initiating bootstrap from ' + targetNode);
	...
	_handleBootstrapRequest : function(msg, msginfo, forwarding) {
		winston.verbose('Bootstrap message for joining node ' + msg.content.joining_node_id + ' from ' + msg.source_id + ' (' + msginfo.source_ap + ')');		
		...
		if (isThisNodeNearest) {
			...
			winston.info('Going to re-broadcast bootstrapping request into the ring on behalf of joining node ' + joiningNodeId);
	...
	_handleBootstrapResponse : function(msg, msginfo) {
		winston.verbose('Bootstrap response from ' + msg.source_id + ' (' + msginfo.source_ap + ')');
// TODO: ensure recipient gets to know its public ip from response (perhaps add a to header to resp)		
		if (msg.content.last_bootstrap_hop && self.bootstrapping) {
			winston.info('=== Bootstraping completed for node ' + node.nodeId + ' ===');
			...
			winston.warn('Unexpected bootstrap response from ' + msg.source_id);
		}
	},
	...
	_sendStateTablesToPeers : function() {
		...
		// do the sending
		winston.verbose('Sending state tables to leafset and routing table peers ' + Object.keys(destinations) + ' upon bootstrap completion');
