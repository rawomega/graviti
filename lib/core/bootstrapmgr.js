...
var self = module.exports = {
	...
	_handleBootstrapRequest : function(msg, msginfo, forwarding) {
		...
		var isThisNodeNearest = leafsetmgr.isThisNodeNearestTo(joiningNodeId);
		...
		leafsetmgr.updateWithProvisional(msg.content.id, bootstrapSourceAddr + ':' + bootstrapSourcePort);
	...
	_handleBootstrapResponse : function(msg, msginfo) {
		...
		leafsetmgr.updateWithKnownGood(msg.content.id, msginfo.sender_addr + ':' + msginfo.sender_port);
	...
	_sendStateTablesToPeers : function() {
		var peers = [];
		leafsetmgr.each(function(id, peer) {
