...
var self = module.exports = {
	...
	_handleBootstrapRequest : function(msg, msginfo, forwarding) {
		...
		var sharedRoutingTableRow = routingmgr.getSharedRow(joiningNodeId);
	...
	_handleBootstrapResponse : function(msg, msginfo) {
		...
		routingmgr.updateRoutingTable(msg.source_id, msginfo.sender_addr + ':' + msginfo.sender_port);
	...
	_sendStateTablesToPeers : function() {
		// send leafset to each leafset peer
		leafset.each(function(id, peer) {
			var ap = peer.ap.split(':');
			self.overlayCallback.sendToAddr('p2p:graviti/peers', {
					leafset : leafset.compressedLeafset()
				},
				{method : 'POST'}, ap[0], ap[1]);
		});
		
		// send each routing table row to each peer in it 
		routingmgr.eachRow(function(rowIndex, row) {
			var routingRow = {};
			routingRow[rowIndex] = row;
			Object.keys(row).forEach(function(digit) {
				var ap = row[digit].ap.split(':');
				self.overlayCallback.sendToAddr('p2p:graviti/peers', {
