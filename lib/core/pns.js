...
var self = module.exports = {
	...
	findNearestNode : function(seedNodeAp, joiningNodeId, success, error) {		
		...
		self._inProgress[reqId].timerId = setTimeout(function() {
			winston.info('Bootstrap timed out whilst trying to locate nearest node via seed ' + seedNodeAp);
	...
	cancelAll : function() {
		winston.info('Cancelling all existing PNS requests');
	...
	_handlePnsLeafsetRequest : function(msg, msginfo) {
		winston.verbose('PNS bootstrap request for leafset received');
	...
	_handlePnsLeafsetResponse : function(msg, msginfo) {
		winston.verbose('PNS bootstrap response with leafset received');
		var reqId = msg.content.req_id;
	...
	_handlePnsRoutingRowRequest : function(msg, msginfo) {
		winston.verbose('PNS routing row request received');
		var reqId = msg.content.req_id;
	...
	_handlePnsRoutingRowResponse : function(msg, msginfo) {
		winston.verbose('PNS routing row response received');		
		var reqId = msg.content.req_id;
		var req = self._inProgress[reqId];
		if (req === undefined)
			return;
	...
	_handleRttProbeRequest : function(msg, msginfo) {
		winston.verbose('PNS RTT probe request received');
		var reqId = msg.content.req_id;
		var ap = msginfo.source_ap.split(':');
	...
	_handleRttProbeResponse : function(msg, msginfo) {
		winston.verbose('PNS RTT probe response received');
		var reqId = msg.content.req_id;
		var req = self._inProgress[reqId];
		if (req === undefined)
			return;
	...
	_reportSuccess : function(reqId, bestId, bestAp, bestRtt) {
		...
		winston.verbose('PNS nearby node search completed for seed ' + req.originalSeedAp + ' - returning node ' + bestId + ' (' + bestAp + '), rtt=' + bestRtt);
		if (success)
			success({
				id : bestId,
				ap : bestAp,
