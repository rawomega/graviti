...
// TODO: actually lets allow the seed to be added because it may look different anyway, eg
// may have been passed in as a hostname but coming back as an ip 
		if (ap !== req.seed && req.discoveredPeers.indexOf(ap) < 0)
	...
	_reportSuccess : function(reqId, bestNodeId, bestNodeAp) {
		...
		util.log('PNS nearby node search completed for seed ' + req.seed + ' - returning node ' + bestNodeId + ' (' + bestNodeAp + ')');
		if (success)
			success(bestNodeId, bestNodeAp, discoveredPeers);
	}
};