...
var self = module.exports = {
	...
	_singleRun : function(seed, state, success) {
		...
		winston.verbose('Starting PNS run from seed ' + seed + ' with state ' + JSON.stringify(state));		
		state.used_seeds.push(seed);		
		
		pns.findNearestNode(seed, node.nodeId, function(res) {
			winston.verbose('PNS via seed ' + seed + ' gave nearest node ' + JSON.stringify(res));
	
			if (state.nearest === undefined || state.nearest.rtt > res.rtt) {
				winston.verbose('PNS found new nearest node ' + res.ap + ' with rtt ' + res.rtt + ', replacing ' + JSON.stringify(state.nearest));
				state.nearest = {
					id : res.id,
					ap : res.ap,
					rtt : res.rtt
