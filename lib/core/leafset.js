...
var self = module.exports = langutil.extend(new events.EventEmitter(), {
	...
	updateWithKnownGood : function(a, b) {
		...
		self._update(a,b, function(id, addrPort) {
			if (self._deadset[id] !== undefined) {
				winston.verbose('Found known good peer ' + id + ' in dead set, removing so it can be moved to leafset');
				delete self._deadset[id];
			}
			if (self._candidateset[id] !== undefined) {
				winston.verbose('Found known good peer ' + id + ' in candidate set, removing so it can be moved to leafset');
				delete self._candidateset[id];
			}
			
			if (self._leafset[id]) {
				winston.verbose('Updating node ' + id + ' in leafset');
				self._put(id, addrPort);
			} else if (Object.keys(self._leafset).length < self.leafsetSize) {
				winston.verbose('Adding node ' + id + ' to leafset');
				...
				winston.verbose('Evicting node ' + idToReplace + ' from leafset and replacing with ' + id);
	...
	updateWithProvisional : function(a,b) {
		self._update(a,b, function(id, addrPort) {
			if (self._deadset[id]) {
				winston.verbose('Ignoring dead peer ' + id);
				return;
			}
			...
			if (self._candidateset[id]) {
				if (self._candidateset[id].ap === addrPort)
					...
					winston.verbose('Found provisional peer ' + id + ' with different ip (' + addrPort
			...
			winston.verbose('Adding node ' + id + ' to candidateset');
	...
	clearTimedOutPeers : function() {
		self.each(function(id, item) {
			if (item.lastHeartbeatReceived < (Date.now() - self.timedOutPeerIntervalMsec)) {
				winston.verbose('Found timed out peer ' + id + ' (' + item.ap + ')');
				self.removePeer(id);				
			}
		});
	},
	...
	removePeer : function(id) {
		winston.verbose('Removing peer ' + id + ' from leafset');
		var removed = false;
