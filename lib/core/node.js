...
//		});
	...
	receiveData : function(data, remoteAddress, inProgressState) {		
		var ackId = messenger.parseAck(data);
		if (ackId) {
		...
		var parsed = messenger.progressiveParse(data, inProgressState);
		if (!parsed.content_processed) {
			return parsed;
		}

		parsed.headers['method'] = parsed.method;
		var msg = new messenger.Message(parsed.uri, parsed.content, parsed.headers); 		
		...
		connmgr.send(msg.source_port, msg.source_addr, 'ACK ' + msg.msg_id);
