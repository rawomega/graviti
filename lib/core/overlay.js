...
var self = module.exports = langutil.extend(new events.EventEmitter(), {
	...
	sendToAddr : function(destUri, content, headers, addr, port) {
		var msg = new messenger.Message(destUri, content, headers);
	...
	_send : function(destUri, content, headers, addr, port, destId) {
		...
		var msg = new messenger.Message(destUri, content, headers, destId);
	...
	_processMessage : function(msg, msginfo) {
		...
		if (msg.dest_id !== undefined && msg.dest_id.length > 0) {
			var isThisNodeNearest = leafset.isThisNodeNearestTo(msg.dest_id);			
			nextHop = routingmgr.getNextHop(msg.dest_id);
			if (!isThisNodeNearest && nextHop.id !== node.nodeId) {
				winston.verbose((msg.source_id === node.nodeId ? 'Outbound' : 'En route') + ' forwarding message ' + msg.msg_id + ' to ' + msg.dest_id);
				isForThisNode = false;
			}
		}
		...
		if (isForThisNode) {
			winston.verbose('message for this node: uri ' + msg.uri + ', source ' + msg.source_id);
			...
			if (msg.source_id !== node.nodeId) {				
				...
				if (betterRoute !== undefined) {
					winston.verbose('Found better route (' + betterRoute.id + ') for message from ' + msg.source_id + ' to ' + msg.dest_id + ', going to offer it back');
