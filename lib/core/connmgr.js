...
var self = module.exports = langutil.extend(new events.EventEmitter(), {
	...
	listen : function(port, bindAddr, opts) {
		var listenCallback = function() {
			...
			winston.info('Server listening on ' + svraddr.address + ':' + svraddr.port);
		...
		self.server.on('error', function (e) {
			if (e.code == 'EADDRINUSE') {
				winston.warn('Address in use -- will retry in ' + self.addrInUseRetryMsec + 'ms...');
				setTimeout(function () {
					try {
						self.server.close();
					} catch (e) {
						winston.warn('Server did not close cleanly upon detection of addr in use: ' + e);
					}
					self.server.listen(port, bindAddr, listenCallback);
				}, self.addrInUseRetryMsec);
			} else if (e.code === 'ECONNREFUSED' || e.code === 'ECONNRESET')
				winston.verbose('Connection error: ' + e.code);
			else
				winston.error(e);
		});
	...
	_processReceivedData : function(socket, data) {
		...
		try {
			...
			winston.warn('Failed to parse message from ' + socket.remoteAddress + ': ' + e + '\nReceived:\n' + data);
			return;
		}
		winston.verbose('Message from ' + socket.remoteAddress + ':' + msg.sender_port + ' :\n' + data);
	...
	send : function(port, host, data) {
		...
		client.addListener("data", function(data) {
			winston.warn('Client conn unexpectedly received data: ' + data);
		});
	...
	stopListening : function() {
		...
		try {
			...
			winston.error('Error closing listener: ' + e);
		}
	}
});