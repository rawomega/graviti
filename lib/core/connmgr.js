...
var self = module.exports = langutil.extend(new events.EventEmitter(), {
			...
			if (e.code == 'EADDRINUSE') {
				...
				setTimeout(function () {
					try {
						...
						util.log('Server did not close cleanly upon detection of addr in use: ' + e);
					}
	...
	_processReceivedData : function(socket, data) {
		...
		try {
			...
			util.log('ERROR parsing message from ' + socket.remoteAddress + ': ' + e + '\nReceived:\n' + data);
			return;
		}
		util.log('Message from ' + socket.remoteAddress + ':' + msg.source_port + ' :\n' + data);
	...
	stopListening : function() {
		...
		try {
			...
			util.log('Error closing listener: ' + e);
		}
	}
});