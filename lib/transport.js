...
exports.createTransports = function(port, bindAddr, router) {
	var udptran = new UdpTran(port, bindAddr);
	var tcptran = new TcpTran(port, bindAddr);
	return new Transport(udptran, tcptran, router);
};

function Transport(udptran, tcptran, router) {
...
Transport.prototype.start = function() {
	...
	this.tcptran.start(self.receiveRaw, function() {
		tcpStarted = true;
		if (udpStarted)
			self.emit('ready');
	});
	this.udptran.start(self.receiveRaw, function() {
		udpStarted = true;
		if (tcpStarted)
			self.emit('ready');
	});
...
Transport.prototype.send = function(destUri, content, headers) {
	this._send(destUri, content, headers);
};

//
//Send message directly to a specific known addr and port. Mainly for internal use 
Transport.prototype.sendToAddr = function(destUri, content, headers, addr, port) {
...
Transport.prototype.sendToId = function(destUri, content, headers, destId) {
	this._send(destUri, content, headers, undefined, undefined, destId);
};

//
//Internal send
Transport.prototype._send = function(destUri, content, headers, addr, port, destId) {
	if (destId === undefined)
...
Transport.prototype._processMessage = function(msg, msginfo) {
...
Transport.prototype.sendMessage = function(port, host, msg) {
...
Transport.prototype.receiveRaw = function(data, remoteAddress, inProgressState) {		
