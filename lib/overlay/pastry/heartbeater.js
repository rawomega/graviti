...
Heartbeater = function(overlayCallback, leafset, routingtable) {
	var self = this;
	...
	this.overlayCallback.on('graviti-message-received', function(msg, msginfo) {
...
Heartbeater.prototype._sendPendingHeartbeats : function() {
	this.leafset.each(this._sendHeartbeatToLeafsetPeer);
	this.leafset.eachCandidate(this._sendHeartbeatToLeafsetPeer);			
};
...
Heartbeater.prototype.sendHeartbeatToAddr = function(addr, port, content) {
	this.overlayCallback.sendToAddr('p2p:graviti/heartbeat', content, {method : 'POST'}, addr, port);
};
