var logger = require('logmgr').getLogger('testability/multinode');
		...
		return {
			...
			start : function() {
				...
				var logFunc = function(data) {
					...
					str.split(/[\n\r]/).forEach(function(row) {
						console.log('[' + id.toShortString(nodeId) + '] : ' + row);						
					});
					
					// see if this is telling us that the test server has been bound and if so report
					if (str.indexOf('Test server started on port ' + (conf.base_test_port + idx)) > -1) {
						if (conf.testServerStarted)
							conf.testServerStarted(idx);
					}
				};
			...
			waitUntil : function(operator, expected, func, callback, errCallback, waitedSoFar) {
				if (waitedSoFar !== undefined && (waitedSoFar > conf.wait_timeout_msec)) {
					errCallback("Timed out waiting for given func on node " + idx + " to equal " + expected);
					return;
				}
