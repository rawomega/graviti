var net = require('net');
var logger = require('logmgr').getLogger('testability/evalclient');
...
var self = module.exports = {
	eval : function(f, opts) {
		...
		client.addListener("error", function(e) {
			logger.verbose('test client error: ' + e);
			if (opts.error)
				opts.error(e);
		});
		...
		client.addListener("data", function(data) {
			...
			if (client.timeoutId === undefined)
				client.timeoutId = setTimeout(function() {
					logger.error('Closing hanging client connection - prob stuck on bad data: ' + client.inProgress);
			...
			try {
				...
				logger.info('Looks like packet frag: ' + raw);
		...
		client.addListener("close", function() {
			//logger.verbose('test client connection closed');
		});
	}
};