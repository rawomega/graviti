...
exports.MessageParser.prototype = {
	...
	progressiveParse : function(str, progress) {
		...
		this._parseMethodAndUri(progress);
		this._parseHeaders(progress);
		this._parseContent(progress);
				
		this._clearOutEmptyUnparsedPart(progress);
		return progress;
	},
	...
	_parseMethodAndUri : function(progress) {		
		if (progress.method === undefined) {
			...
			var firstLine = this._parseFirstLine(progress.unparsed_part.substring(0, breakIdx));
	...
	_parseHeaders : function(progress) {
		if (!progress.headers_processed) {
			...
			while(progress.unparsed_part.indexOf('\n') > -1) {
				...
				var nv = this._parseHeaderLine(headerLine);
