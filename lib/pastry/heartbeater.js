var logger = require('logmgr').getLogger('pastry/heartbeater');
var langutil = require('langutil');

Heartbeater = function(transport, leafset, routingtable) {
	this.transport = transport;
	this.leafset = leafset;
	this.routingtable = routingtable;
	this.heartbeatCheckIntervalId = undefined;

	this.heartbeatCheckIntervalMsec = 1000;
	this.heartbeatIntervalMsec = 10000;
	this.timedOutPeerCheckIntervalMsec = 2500;
	this.routingTableCandidateCheckIntervalMsec = 1000;
	this.routingTableMaintenanceIntervalMsec = 15 * 60 * 1000;
...
Heartbeater.prototype.stop = function(notifyPeers) {
	...
	if (notifyPeers) {
		self.leafset.each(function(id, item) {
			...
			self.transport.sendToAddr('p2p:graviti/peers/' + self.transport.nodeId, undefined, {method : 'DELETE'}, ap[0], ap[1]);			
		});			
	}
	
	self.transport.removeListener('graviti-message-received', self._handleReceivedGravitiMessage);
};
...
Heartbeater.prototype._sendHeartbeatToLeafsetPeer = function(id, peer) {
	if (peer.lastHeartbeatSent > (Date.now() - this.heartbeatIntervalMsec))
		return;
...
Heartbeater.prototype.sendHeartbeatToAddr = function(addr, port, content) {
	this.transport.sendToAddr('p2p:graviti/heartbeat', content, {method : 'POST'}, addr, port);
};
