...
Router.prototype.getNextHop = function(id) {
	var self = this;
    logger.verbose('beginning routing for ' + id + ' - leafset is ' + JSON.stringify(self.leafset.compressedLeafset()));
	if (this.leafset.nodeId === id)
		return {
			id   : this.leafset.nodeId
		};
	...
	var commonPrefix = ringutil.getCommonPrefixLength(this.leafset.nodeId, id);			
	...
	fallbackRoutingCandidates[this.leafset.nodeId] = 'me';
	if (rowForDigit) {
	...
	if (res.nearest === this.leafset.nodeId) {
		return {
			id   : res.nearest
		};	
	} else {
