...
Bootstrapper = function(transport, leafset, routingtable, heartbeater, pns) {	
...
Bootstrapper.prototype.start = function(bootstraps, bootstrapCompletedCallback) {
	...
	if (bootstraps && bootstraps.length > 0) {
		...
		logger.info('=== Going to start a new ring as ' + this.transport.nodeId + ' (empty bootstrap list) ===');
	}
};
...
Bootstrapper.prototype._startBootstrapping = function(bootstraps) {
	...
	self.bootstrappingIntervalId = setInterval(function() {	
		...
		for (var endpoint in self.bootstrapEndpoints) {						
			...
			if (self.usePns) {
				self.pns.run(endpoint, function(ap) {
...
Bootstrapper.prototype._bootstrapFrom = function(targetNode) {
	...
	var content = {
			joining_node_id : this.transport.nodeId
	};
...
Bootstrapper.prototype._handleBootstrapRequest = function(msg, msginfo, forwarding) {
	...
	if (bootstrapRequestHops.indexOf(this.transport.nodeId) < 0)
		bootstrapRequestHops.push(this.transport.nodeId);
...
// TODO: ensure recipient gets to know its public ip from response (perhaps add a to header to resp)		
	if (msg.content.last_bootstrap_hop && this.bootstrapping) {
		logger.info('=== Bootstraping completed for node ' + this.transport.nodeId + ' ===');
		this.bootstrapping = false;
        ...
        logger.warn('Unexpected bootstrap response: msg id ' + msg.msg_id + ', method ' + msg.method + ', uri ' + msg.uri
            + ', source ' + msg.source_id + ', last_bootstrap_hop = '
            + msg.content.last_bootstrap_hop + ', boostrapping = ' + this.bootstrapping);
	}
};
