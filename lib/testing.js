...
exports.createRing = function(opts) {
	...
	for (var i = 0; i < numNodes; i++) {
		...
		var node = pastry.createNode(nodeId, conf.base_port+i, '127.0.0.1');
		if (node.port === conf.base_port)
			node.startRing(function(readyNode) {
				testNodes.push(readyNode);
			...
			node.joinRing('localhost:' + conf.base_port, function(readyNode) {
				testNodes.push(readyNode);
				if (testNodes.length === numNodes) {
					var ring = new TestRing(testNodes);
					opts.success(ring);
		...
		node.eval = function(func, success, error) {
			var res = func(this);
			logger.info('[' + this.port + ']: eval res: ' + res + ' for ' + func.toString());
			success(res);
		};
		...
		node.waitUntil = function(operator, expected, func, callback, errCallback, waitedSoFar) {
			if (waitedSoFar !== undefined && (waitedSoFar > conf.wait_timeout_msec)) {
				errCallback("Timed out waiting for given func on node " + this.port + " to equal " + expected);
				return;
			}
...
function TestRing(nodes) {
	...
	this.nodeIds = [];
	for (var n in nodes) {		
		console.log('AAAAAAAAA Ring node: ' + JSON.stringify(nodes[n].nodeId));
