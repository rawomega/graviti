...
exports.createRing = function(opts) {
	...
	for (var i = 0; i < numNodes; i++) {
        ...
        testNodes.push(node);        
        if (opts.nodeCreated)
            opts.nodeCreated(node);
    
        function nodeReadyHandler(readyNode) {
            numReadyNodes++;
            if (numReadyNodes === numNodes) {
		...
		if (node.port === conf.base_port)
            node.startRing(nodeReadyHandler);
		else
            node.joinRing('localhost:' + conf.base_port, nodeReadyHandler);
