...
var self = module.exports = langutil.extend(new events.EventEmitter(), {	
	...
	_startNode : function(port, bindAddr, opts) {
		...
		self.on('graviti-message-forwarding', self._handleReceivedGravitiMessage);
	},
	...
	_handleBootstrapRequest : function(msg, msginfo, forwarding) {
		...
		var iAmNearest = ringutil.amINearest(joiningNodeId, node.nodeId, leafsetmgr.leafset);
		if (iAmNearest) {
			content.last_bootstrap_hop = true; 
		} else if (!forwarding) {
			self.send('p2p:graviti/statetables', msg.content, {method : 'GET'});
		}
		...
		// and only now add new node to leafset, otherwise we may end up using it for routing!
		leafsetmgr.updateLeafset(msg.content.id, msg.bootstrap_source_addr + ':' + msg.bootstrap_source_port); 
	},
