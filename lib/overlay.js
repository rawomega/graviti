...
var self = module.exports = langutil.extend(new events.EventEmitter(), {	
	...
	sendToAddr : function(destUri, content, headers, addr, port) {
		self._send(destUri, content, headers, addr, port, undefined);
	},
	...
	_startNode : function(port, bindAddr, opts) {
		...
		//self.on("graviti-message-received", function() {console.log('aaa')});
	},
	...
	_processMessage : function(msg, msginfo) {
		...
		var isForMe = ringutil.isForMe(msg.dest_id, node.nodeId, self.leafset);		
		...
		if (isForMe) {
			...
			var nextHop = mapIdToNode();	// todo: replace with correct call when implemented			
